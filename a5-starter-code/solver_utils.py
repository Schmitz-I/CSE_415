...
def value_iteration(
    ...
    for s in mdp.nonterminal_states:
        ...
        for a in mdp.actions:
            q_value = sum( mdp.transition(s, a, next_state) * 
                          (mdp.reward(s, a, next_state) + mdp.config.gamma * v_table[next_state]) 
...
def extract_policy(
    ...
    #return {s: max([op.name for op, _ in mdp.state_graph[s]], key=lambda a: q_table[(s, a)]) for s in mdp.nonterminal_states}
...
def q_update(
    ...
    if next_state == mdp.terminal:
        ...
        for a in mdp.actions: 
            q_val = q_table.get((next_state, a), 0.0)
            if q_val > best:
                best = q_val


    q_table[(state, action)] = (1 - alpha) * q_table[(state, action)] + alpha * (reward + gamma * best)
